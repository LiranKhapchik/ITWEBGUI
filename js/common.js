const SVG_ICONS = {
    ...
    'dashboard': `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h7v9H3z M14 3h7v5h-7z M14 12h7v9h-7z M3 16h7v5H3z"/>`,
...
// --- SSO User Session Initialization ---
window.currentUser = null;
window.ssoUserPromise = null;

function blockUnregisteredUser(user) {
    document.body.innerHTML = `
        <div style="position: fixed; inset: 0; background: linear-gradient(135deg, #0f172a, #1e293b); color: white; display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 999999; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; text-align: center; padding: 20px;" dir="rtl">
            <div style="background: rgba(255,255,255,0.03); border: 1px rgba(255,255,255,0.1) solid; backdrop-filter: blur(10px); padding: 40px; border-radius: 24px; box-shadow: 0 20px 50px rgba(0,0,0,0.3); max-w: 450px; width: 100%;">
                ...
                <h2 style="font-size: 22px; font-weight: 800; margin-bottom: 10px;">שגיאת גישה - המערכת ממודרת</h2>
                <p style="font-size: 14px; color: #94a3b8; font-weight: 600; line-height: 1.6; margin-bottom: 24px;">המערכת מיועדת למשתמשים מורשים בלבד. אינך רשום במערכת ה-SSO.<br>פנה למנהל המערכת על מנת להסדיר את גישתך.</p>
                <div style="font-size: 11px; font-weight: bold; color: #64748b; background: rgba(0,0,0,0.2); padding: 10px; border-radius: 12px; font-family: monospace;">משתמש AD: ${user ? user.username : 'לא ידוע'}</div>
            </div>
        </div>
    `;
    throw new Error("Access Denied: Unregistered user.");
}

async function loadSsoUser() {
    if (window.currentUser) {
        if (!window.currentUser.is_registered) blockUnregisteredUser(window.currentUser);
        return window.currentUser;
    }
    
    const cached = localStorage.getItem('sso_user');
    if (cached) {
        try { 
            const parsed = JSON.parse(cached);
            if (parsed && parsed.username && parsed.username !== parsed.username.toLowerCase()) {
                localStorage.removeItem('sso_user');
            } else {
                window.currentUser = parsed;
                if (parsed && !parsed.is_registered) blockUnregisteredUser(parsed);
            }
        } catch(e) {}
    }
    
    try {
        const url = 'api.php?file=get_current_user';
        ...
        if (res.ok) {
            ...
            if (user && user.username) {
                ...
                for (let key in localStorage) {
                    if (key.startsWith('mch_area_grant_')) {
                ...
                if (user.permissions && Array.isArray(user.permissions)) {
                    user.permissions.forEach(p => {
                        localStorage.setItem('mch_area_grant_' + p, 'true');
                    });
                }
...
function hasAreaAccess(areaId) {
    if (localStorage.getItem('mch_authorized') === 'true') return true;
    return localStorage.getItem('mch_area_grant_' + areaId) === 'true';
}
window.hasAreaAccess = hasAreaAccess;

window.ssoUserPromise = loadSsoUser();
